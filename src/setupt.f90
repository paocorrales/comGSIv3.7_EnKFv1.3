...
! Declare local variables
  ...
  real(r_kind) psges,sfcchk,pres_diff,rlow,rhgh,ramp,zsges
...
!       Load observation value and observation error into local variables
        ...
        !write(*,*)'linea 446: obs=',tob,' obserror=',obserror
     endif

! For obs type=287, check if elv~ges_elv and use psges as dpres
     
     if (itype == 187) then
             !write(*,*)'Entra al if para 187'
       call tintrp2a11(ges_z,zsges,dlat,dlon,dtime,hrdifsig,&
                     mype,nfldsig)
             !write(*,*)'ges_z=',ges_z,' zsges=',zsges
       if ( abs(zsges-data(iobshgt,i))<200 ) then
               !write(*,*)'Entra al if diff menor a 200'
               call tintrp2a11(ges_ps,psges,dlat,dlon,dtime,hrdifsig,&
                       mype,nfldsig)
               dpres=log(psges)      !dpres in log of cb
               !prest=r10*exp(dpres)  ! in mb
               !write(*,*)'Nuevo dpres para type=',itype,' dpres=',dpres
       endif
     endif
...
! Interpolate log(ps) & log(pres) at mid-layers to obs locations/times
     ...
     if(sfctype .and. .not.twodvar_regional) then
        ...
        !write(*,*)'Para el cálculo de drpx con la obs typo=',itype
        !write(*,*)'dpres=',dpres
        !write(*,*)'psges=',psges
        !write(*,*)'rd_over_cp=',rd_over_cp
        !write(*,*)'t0c=',t0c
        !write(*,*)'Entonces drpx=',drpx
     end if

!    Put obs pressure in correct units to get grid coord. number
     !write(*,*)'Antes de entrar al grdcrd1: dpres=',dpres,' prsltmp=',prsltmp(1),' nsig=',nsig
     call grdcrd1(dpres,prsltmp(1),nsig,-1)
     !write(*,*)'dpres after grdcrd1=',dpres
...
!    Check to see if observations is above the top of the model (regional mode)
     if(sfctype)then
             !write(*,*)'dpres before if>4 =',dpres
        if(abs(dpres)>four) drpx=1.0e10_r_kind
        pres_diff=prest-r10*psges
        !write(*,*)'Inside abs(dpres>4) drpx queda=',drpx,'y pres_diff=',pres_diff
        if (twodvar_regional .and. abs(pres_diff)>=r1000) drpx=1.0e10_r_kind
        !write(*,*)'If 2dvar and pres_diff>1000, entonces drpx=',drpx,'twodvar_regional=',twodvar_regional
     end if
...
! linear variation of observation ramp [between grid points 1(~3mb) and 15(~45mb) below the surface]
     ...
     !write(*,*)'type=',itype
     !write(*,*)'Linea 741 en adelante: error=', error 
     ratio_errors=error/(data(ier,i)+drpx+1.0e6_r_kind*rhgh+r8*ramp)
     error=one/error
     !write(*,*)'ratio_errors=',ratio_errors,' error(1/error)=',error
     !write(*,*)'drpx=',drpx
     !write(*,*)'rhgh=',rhgh
     !write(*,*)'r8=',r8
     !write(*,*)'ramp=',ramp
!    if (dpres > rsig) ratio_errors=zero
     if (dpres > rsig )then
        if( regional .and. prest > pt_ll )then
           ...
           ratio_errors=zero
           !write(*,*)'If dpres>rsing & regional, ratio_errors=',ratio_errors
        endif
     endif
...
!    Gross error checks
     ...
     !write(*,*)'gross error checks:'
     !write(*,*)'obserror=',obserror
     !write(*,*)'obserrlm=',obserrlm
     !write(*,*)'residual=',residual
     !write(*,*)'ratio=',ratio
     !write(*,*)'ratiosfc=',ratiosfc
...
! extend the threshold for surface T
        ...
        !write(*,*)'if sfctype: error=',error,' ratio_errors=',ratio_errors
     else
        ...
        !write(*,*)'else sfctype: error=',error,' ratio_errors=',ratio_errors
     endif
     
     if (ratio_errors*error <=tiny_r_kind) muse(i)=.false.
     !write(*,*)'linea 859: tiny_r_kind=',tiny_r_kind
...
! Save select output for diagnostic file
     if (conv_diagsave .and. luse(i)) then
        ...
        !write(*,*)'select output for diafnostic file:'
        !write(*,*)'luse=',luse(i)
        !write(*,*)'err_input=',err_input
        !write(*,*)'err_adjst=',err_adjst
        !write(*,*)'err_final=',err_final
        !write(*,*)'errinv_input=',errinv_input
        !write(*,*)'errinv_adjst=',errinv_adjst
        !write(*,*)'errinv_final=',errinv_final
...
! Save select output for diagnostic file
           if (conv_diagsave .and. luse(i)) then
              ...
              if(iip <= 3*nobs) then
                 ...
                 !write(*,*)'err_adjst=',err_adjst
                 !write(*,*)'err_final=',err_final
                 !write(*,*)'errinv_input=',err_input
                 !write(*,*)'errinv_adjst=',err_adjst
                 !write(*,*)'errinv_final=',err_final
...
! in the format of:  var_jb*1.0e+6 + rwgt
    ...
    !write(*,*)'write diag:'
    !write(*,*)'rdiagbuf(14,ii) - input=',rdiagbuf(14,ii)
    !write(*,*)'rdiagbuf(15,ii) - adjst=',rdiagbuf(15,ii)
    !write(*,*)'rdiagbuf(16,ii) - final=',rdiagbuf(16,ii)
    if (aircraft_t_bc_pof .or. aircraft_t_bc .or. aircraft_t_bc_ext) then
